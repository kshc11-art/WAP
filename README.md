...
## 보조원장 지재권 집행내역 v3.3.1 — 검증 중

[설치용 userscript](scripts/stats-ledger.user.js)를 제공 번들의 v3.3을 기준으로 갱신했습니다. v3.3.1은 대괄호 적요의 누락 분류를 review에 표시하고, 다른 번호 속 KR 출원번호 오인과 PCT·소프트웨어 저작권의 국내외 충돌을 보정합니다. 엑셀 업로드·백데이터·통계 출력은 외부 CDN 없이 동작하도록 제공된 구조를 유지합니다. 백업 시 비활성 상태는 설치 권장이나 회사 PC 활성화 상태를 뜻하지 않습니다.

금액 기준은 대체·환원 전표를 제외한 각 행의 차변금액−대변금액입니다. 관리번호가 없으면 관리번호별 통계에서 `(관리번호 없음)`으로 합쳐지므로 개별 특허 비용 확정에 사용하기 전에 원장 키를 확인해야 합니다. `tests/test_stats_ledger.cjs`는 합성 행별 금액·제외 건수·고유키·XLSX 왕복과 분류 충돌을 검증합니다.

제공된 결과 XLSX의 행별 산술은 로컬에서 대조했지만, 번들에는 테스트가 읽는 원본 보조원장 XLSX가 없어 입력→정제 과정과 제외 근거는 재현하지 못했습니다. 해당 결과를 golden으로 지정하지 않았고 저장소에 올리지 않았습니다. 화면 조회의 전량 수신, 키 충돌, 비정상 금액 입력, 사내망·Office 출력 및 사용자 지정 raw/golden 비교는 확인 대기입니다.
