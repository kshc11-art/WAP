...
## 보조원장 지재권 집행내역 v3.3.2 — 검증 중

[설치용 userscript](scripts/stats-ledger.user.js)는 제공된 v3.3을 기준으로 갱신했다. v3.3.1에서 대괄호 적요의 분류 누락을 review에 표시하고 번호 경계·PCT·소프트웨어 저작권의 국내외 충돌을 보정했다. v3.3.2는 명시된 30/40 계열 출원·등록번호를 무조건 특허로 추론하지 않고 `기타/미확인`과 검토 사유로 남긴다. 해당 번호는 제공 상세목록과 결합되는 디자인·상표 비용에서 실제 특허 오분류 근거로 확인했다. 합성 회귀는 `tests/test_stats_ledger.cjs`에서 실행하며 외부 CDN은 도입하지 않았다.

제공된 2026-09-15 보조원장을 엑셀 업로드 경로로 로컬 실행해 입력·제외·보존 행수와 차변−대변을 독립 합산했다. 기존 2026-08-27 산출물도 이 원장의 해당 기간과 키·금액·파생 열이 일치했다. 이는 이전 스크립트 결과의 재현성이며 권리유형·특허별 귀속의 최종 승인은 아니다. 상세목록과 단일 결합되는 행에서 비특허를 특허로 표시한 경우와 국내외 미분류가 발견됐다. 원장의 관리번호 열은 비어 있고, 현재 스크립트는 적요의 RESI 신청번호를 상세목록의 관리번호로 변환하지 않는다. 그 결과 관리번호별 피벗은 많은 비용을 `(관리번호 없음)`으로 합친다.
