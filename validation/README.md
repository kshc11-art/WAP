...
현재 저장소의 validation/raw·golden은 미제공이다. ZIP에 있던 과거 산출물과 검증 결과를 이번 golden으로 채택하지 않았다.
...
## 보조원장 지재권 집행내역 v3.3.2 — 2026-09-15

제공 번들의 v3.3 userscript를 기존 경로에 반영했다. v3.3.1은 누락 검토·번호 경계·정책 충돌을 보정했고, v3.3.2는 명시적 30/40 계열 출원·등록번호를 `기타/미확인`으로 두고 권리유형 확인을 요구한다. `tests/test_stats_ledger.cjs`는 합성 원본 키·행별 순액·제외분·XLSX 왕복과 이 분류 회귀를 검사한다.

대화로 제공된 보조원장을 로컬 엑셀 업로드 경로에 투입해 산술·입출력 건수와 기존 산출물의 키·금액·파생 열을 재현했다. 상세목록의 고유키 결합에서 실제 비특허→특허 오분류와 국내외 미분류가 확인됐으며, 번호 보정만으로 상세목록 결합을 대체할 수 없다. 원장 관리번호 열은 공백이고 현재 관리번호별 통계는 RESI 신청번호를 상세목록 관리번호로 변환하지 않아 미귀속 비용을 한 버킷으로 합친다. 원자료는 Git에 넣지 않았고 사용자 지정 golden도 없다.

`대체|환원` 제외가 실제 비용 제거인지 상대계정 없이 검증할 수 없다. 수작업 통계는 기준일·특허 한정 범위·백만원 단위 처리 기준이 확정되지 않아 golden이 아니다. 상세목록 결합의 미결합·다의 키와 내부망 전량조회·Office 출력은 대기다.
...
| `scripts/stats-ledger.user.js` | 보조원장 엑셀 업로드 또는 Kendo/DOM·분할 조회 → 대체/환원 제외, 차변−대변, 사건 분류 → 백데이터·review·통계 XLSX/TSV | 해당 PMS 화면과 jQuery/Kendo; 제공 v3.3.2는 외부 CDN 없음. 백업 시 비활성 | 원본→출력 전량, 비정상 금액·중복키, 제외 전표 정책, 관리번호 공백의 RESI→상세목록 결합 및 비특허 오분류. `순번` 헤더 2개는 원본순번의 위치를 구분해 검사 |
