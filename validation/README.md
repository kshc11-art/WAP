...
## 보조원장 지재권 집행내역 v3.3.1 — 2026-09-15

제공 번들의 userscript v3.3을 `scripts/stats-ledger.user.js`에 반영하고 네 가지 누락·충돌 분류를 v3.3.1에서 보정했다. `tests/test_stats_ledger.cjs`는 합성 행의 원본 키·보존 행수·행별 순액·제외분과 XLSX 메모리 왕복, 대괄호 적요 누락 검토·KR 번호 경계·PCT/저작권 수기 충돌을 검사한다. 총합을 동일 레코드에서 두 번 계산하는 검사는 보조 검사다.

첨부된 결과 XLSX는 차변−대변 산술을 로컬 대조하는 자료로만 사용했다. 테스트가 필요로 하는 원본 `보조원장_20260826182259.xlsx`는 번들에 없으며, 결과물을 사용자 지정 golden으로 채택하지 않았다. 입력 4,650행에서 출력 4,508행으로 바뀐 근거와 실제 사건별 비용·제외 전표 정책은 원본 및 기대 결과 없이는 PASS가 아니다. 관리번호 공백은 관리번호별 통계에서 한 버킷으로 합쳐진다. 사내망 전량 조회·중복키 충돌·비정상 금액·Office 출력은 확인 대기다.
...
| `scripts/stats-ledger.user.js` | 보조원장 엑셀 업로드 또는 Kendo/DOM·분할 조회 → 대체/환원 제외, 차변−대변, 사건 분류 → 백데이터·review·통계 XLSX/TSV | 해당 PMS 화면과 jQuery/Kendo; 제공 v3.3.1은 외부 CDN 없음. 백업 시 비활성 | 원본→출력 전량, 비정상 금액·중복키, 제외 전표 정책, 관리번호 공백의 사건별 귀속. `순번` 헤더 2개는 원본순번의 위치를 구분해 검사 |
